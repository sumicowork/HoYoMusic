      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {credits.map((credit) => {
          ...
          return (
            <article
              key={credit.id}
              className="rounded-2xl border border-white/[0.12] bg-black/25 p-4 transition-transform duration-200 hover:-translate-y-0.5"
            >
              <p className="text-[11px] uppercase tracking-[0.2em] text-white/55">{credit.credit_key}</p>
                ...
                {people.length > 0 ? (
                  people.map((name) => (
                    <button
                      key={`${credit.id}-${name}`}
                      type="button"
                      className="credit-link h-11 rounded-full border border-white/15 bg-white/8 px-3 text-sm text-white/90 transition-colors hover:border-indigo-200/45 hover:text-indigo-100"
                  ...
                  <span className="text-sm text-white/65">{credit.credit_value}</span>
                )}
              </div>
            </article>
          );
